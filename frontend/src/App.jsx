...
function ScoreRing({ score, isAnomaly }) {
  ...
  const pct = clamp(score, 0, 1), dash = pct * circ;
  const color = isAnomaly ? "#ff2d2d" : score > 0.25 ? "#ffaa00" : "#00e5a0";
  return (
    <div style={{ position: "relative", width: 140, height: 140 }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 26, fontWeight: 700, color, lineHeight: 1 }}>{fmt(score)}</span>
...
/* ── Sparkline ── */
function Sparkline({ data, isAnomaly, threshold }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: 64 }} preserveAspectRatio="none">
      ...
      <line x1="0" y1={H - (threshold / max) * (H - 8) - 4}
        x2={W} y2={H - (threshold / max) * (H - 8) - 4}
...
function AlertRow({ alert, idx }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 12px",
      ...
      borderLeft: idx === 0 ? "2px solid #ff2d2d" : "2px solid transparent", transition: "all 0.3s" }}>
      ...
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 13, fontWeight: 700,
        color: alert.score > 0.7 ? "#ff2d2d" : "#ffaa00" }}>{alert.score.toFixed(3)}</span>
    </div>
  );
}
...
function DangerBadge({ type }) {
  const cfg = {
    crime:  { color: "#ff2d2d", label: "CRIME" },
    weapon: { color: "#ff8c00", label: "WEAPON" },
    both:   { color: "#cc00ff", label: "CRIME+WEAPON" },
  }[type] ?? { color: "#4a5568", label: type };
  return (
    <span style={{ background: cfg.color + "22", border: `1px solid ${cfg.color}`, color: cfg.color,
    ...
    <div style={{ background: "#0d1117", border: "1px solid #1e2533", borderRadius: 10, overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <div style={{ position: "relative", background: "#000", aspectRatio: "16/9", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", bottom: 6, right: 8,
          ...
          color: event.score > 0.7 ? "#ff2d2d" : "#ffaa00", textShadow: "0 0 6px rgba(0,0,0,0.8)" }}>
          {event.score.toFixed(3)}
        </div>
      </div>
      <div style={{ padding: "10px 12px" }}>
        <div style={{ fontSize: 10, color: "#4a5568", letterSpacing: 1, marginBottom: 4 }}>{event.timestamp}</div>
        <div style={{ fontSize: 12, color: "#e2e8f0", letterSpacing: 1 }}>{event.label}</div>
        {event.crime_type && event.crime_type !== "Unknown" && (
          <div style={{ fontSize: 10, color: "#00e5a0", marginTop: 4, textTransform: "uppercase" }}>🔍 {event.crime_type}</div>
        )}
        {event.weapons.length > 0 && (
          <div style={{ fontSize: 10, color: "#ff8c00", marginTop: 4 }}>🔫 {event.weapons.join(", ")}</div>
        )}
      </div>
    </div>
  );
...
export default function App() {
  ...
  const connectWS = useCallback(() => {
    ...
    ws.onclose = () => { setConnStatus("disconnected"); setTimeout(() => connectWS(), 2000); };
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (d.history_count !== undefined) setHistoryCount(d.history_count);
      } catch {}
    };
    wsRef.current = ws;
  }, []);
  ...
  useEffect(() => {
    pingRef.current = setInterval(() => { if (wsRef.current?.readyState === 1) wsRef.current.send("ping"); }, 5000);
  ...
  const fetchHistory = useCallback(async (page = 0) => {
    const res  = await fetch(`${API}/api/history?limit=${PAGE_SIZE}&offset=${page * PAGE_SIZE}`);
  ...
  useEffect(() => {
    if (activeTab === "history") fetchHistory(0);
  }, [activeTab, fetchHistory]);

  const clearHistory = async () => {
    await fetch(`${API}/api/history`, { method: "DELETE" });
    setHistoryData([]); setHistoryTotal(0); setHistoryPage(0); setHistoryCount(0);
  };
  ...
  return (
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type=text], input[type=number] {
          background: #111827; border: 1px solid #1e2533; color: #e2e8f0;
        ...
        .btn { padding: 10px 24px; border: none; border-radius: 6px; font-family: 'JetBrains Mono', monospace;
          font-size: 13px; font-weight: 700; letter-spacing: 2px; cursor: pointer; transition: all 0.2s; }
        .btn:disabled { opacity: 0.4; cursor: not-allowed; }
        .btn-start { background: #00e5a0; color: #090c14; }
        .btn-start:hover { background: #00ffb3; transform: translateY(-1px); }
        .btn-stop { background: transparent; color: #ff2d2d; border: 1px solid #ff2d2d; }
        .btn-stop:hover { background: rgba(255,45,45,0.1); }
        .btn-ghost { background: transparent; color: #4a5568; border: 1px solid #1e2533; padding: 6px 14px; font-size: 11px; }
        .btn-ghost:hover { border-color: #4a5568; color: #a0aec0; }
        .card { background: #0d1117; border: 1px solid #1e2533; border-radius: 10px; padding: 20px; }
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "14px 28px", borderBottom: "1px solid #1e2533", background: "#090c14",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <div style={{ fontSize: 11, color: "#2d3748" }}>DEVICE: <span style={{ color: "#4a5568" }}>{device}</span></div>
          <div style={{ fontSize: 11, color: "#2d3748" }}>FRAMES: <span style={{ color: "#4a5568" }}>{framesProc.toLocaleString()}</span></div>
        </div>
      </div>

      {/* Tab Bar */}
      <div style={{ display: "flex", borderBottom: "1px solid #1e2533", padding: "0 24px", background: "#090c14" }}>
        {[
          { id: "live",    label: "LIVE FEED",      badge: 0 },
          { id: "history", label: "DANGER HISTORY", badge: historyCount },
        ].map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
            background: "transparent", border: "none",
            borderBottom: activeTab === tab.id ? "2px solid #00e5a0" : "2px solid transparent",
            color: activeTab === tab.id ? "#00e5a0" : "#4a5568",
            fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 700, letterSpacing: 2,
            padding: "10px 20px", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, transition: "all 0.2s",
          }}>
            {tab.label}
            {tab.badge > 0 && (
              <span style={{ background: "#ff2d2d", color: "#fff", fontSize: 9, padding: "1px 6px", borderRadius: 10 }}>
                {tab.badge}
              </span>
            )}
          </button>
      ...
      {/* ── LIVE TAB ── */}
      {activeTab === "live" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 16, padding: "16px 24px", height: "calc(100vh - 105px)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, overflow: "hidden" }}>
            <div className="card" style={{ flex: "1 1 auto", overflow: "hidden", position: "relative", minHeight: 0 }}>
              ...
              {running ? (
                ...
                <div style={{ height: "calc(100% - 24px)", display: "flex", flexDirection: "column",
                  alignItems: "center", justifyContent: "center", background: "#060810", borderRadius: 6, border: "1px dashed #1e2533" }}>
              ...
              {isAnomaly && running && (
                <div style={{ position: "absolute", inset: 0, border: "3px solid #ff2d2d",
                  borderRadius: 10, pointerEvents: "none", animation: "pulse 1s ease-in-out infinite" }} />
              )}
            </div>
            <div className="card" style={{ flex: "0 0 auto" }}>
              ...
              <Sparkline data={scoreHistory} isAnomaly={isAnomaly} threshold={threshold} />
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, overflowY: "auto" }}>
            ...
            <div className="card" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
              ...
              <div style={{ padding: "10px 24px", borderRadius: 6, width: "100%", textAlign: "center", transition: "all 0.3s",
                background: isAnomaly ? "rgba(255,45,45,0.12)" : "rgba(0,229,160,0.08)",
                border: `1px solid ${isAnomaly ? "#ff2d2d" : "#00e5a0"}` }}>
                <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 16, letterSpacing: 2,
                  color: isAnomaly ? "#ff2d2d" : "#00e5a0" }}>
                  {isAnomaly ? "⚠ " + label : label}
                </div>
              </div>
            </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div>
                  ...
                  <input type="text" value={source} onChange={e => setSource(e.target.value)}
                    placeholder="0 | rtsp:// | http:// | file.mp4" disabled={running} />
                </div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                  {!running ? (
                    <button className="btn btn-start" style={{ flex: 1 }} onClick={handleStart}>▶ START</button>
                  ) : (
                    <button className="btn btn-stop" style={{ flex: 1 }} onClick={handleStop}>■ STOP</button>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="card" style={{ flex: 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <div className="label" style={{ margin: 0 }}>
                  ALERT LOG
                  {alerts.length > 0 && (
                    <span style={{ marginLeft: 8, background: "#ff2d2d", color: "#fff", fontSize: 9, padding: "1px 6px", borderRadius: 10 }}>
                      {alerts.length}
                    </span>
                  )}
                </div>
                {alerts.length > 0 && <button className="btn btn-ghost" onClick={clearAlerts}>CLEAR</button>}
              </div>
              <div style={{ overflowY: "auto", maxHeight: 220 }}>
                {alerts.length === 0 ? (
                  <div style={{ textAlign: "center", color: "#2d3748", fontSize: 11, padding: "24px 0", letterSpacing: 2 }}>NO ALERTS</div>
                ) : (
      ...
      {/* ── HISTORY TAB ── */}
      {activeTab === "history" && (
        <div style={{ padding: "20px 24px", height: "calc(100vh - 105px)", display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 800, color: "#e2e8f0" }}>
              Danger History
              <span style={{ marginLeft: 12, fontSize: 12, color: "#4a5568", fontFamily: "'JetBrains Mono', monospace" }}>
                {historyTotal} event{historyTotal !== 1 ? "s" : ""} saved
              </span>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button className="btn btn-ghost" onClick={() => fetchHistory(historyPage)}>↻ REFRESH</button>
              {historyTotal > 0 && <button className="btn btn-stop" onClick={clearHistory}>🗑 CLEAR DB</button>}
            </div>
          </div>

          <div style={{ flex: 1, overflowY: "auto", display: "grid",
            gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 14, alignContent: "start" }}>
            {historyData.length === 0 ? (
              <div style={{ gridColumn: "1/-1", textAlign: "center", color: "#2d3748",
                fontSize: 12, letterSpacing: 3, padding: "60px 0" }}>
          ...
          {totalPages > 1 && (
            <div style={{ display: "flex", justifyContent: "center", gap: 8 }}>
              <button className="btn btn-ghost" onClick={() => fetchHistory(historyPage - 1)} disabled={historyPage === 0}>← PREV</button>
              <span style={{ color: "#4a5568", fontSize: 11, alignSelf: "center", letterSpacing: 2 }}>
                {historyPage + 1} / {totalPages}
              </span>
              <button className="btn btn-ghost" onClick={() => fetchHistory(historyPage + 1)} disabled={historyPage >= totalPages - 1}>NEXT →</button>
            </div>
          )}
        </div>
      )}
